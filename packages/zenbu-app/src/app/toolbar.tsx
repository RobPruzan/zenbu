...
import { Tldraw, track, useEditor, TLEditorComponents, createTLStore, TldrawEditor } from "tldraw";
import "tldraw/tldraw.css";
import { ResizableHandle, ResizablePanel, ResizablePanelGroup } from "~/components/ui/resizable";
import layout from "./layout";

type ToolSection = "select" | "console" | "network" | "more" | "draw" | "record" | "settings" | "performance" | null;
type ToggleableTools = "record" | "draw";
type RecordingState = "idle" | "starting" | "recording" | "stopping" | "recorded" | "error";
type Recording = {
  id: string;
  url: string;
  name: string;
...
const DrawingPanel = ({ editor }: { editor: any }) => {
  ...
  const tools = [
    { id: 'select', label: 'Select', icon: <SquareMousePointer size={14} />, shortcut: 'V' },
    { id: 'draw', label: 'Pen', icon: <Brush size={14} />, shortcut: 'D' },
    { id: 'eraser', label: 'Eraser', icon: <Eraser size={14} />, shortcut: 'E' },
    ...
    const interval = setInterval(() => {
      if (editor && editor.getCurrentToolId) {
        try {
          ...
          const displayToolId = toolId === 'rectangle' || toolId === 'ellipse' ? 'geo' : toolId;
    ...
    <div className="p-3 text-white/70 text-sm h-full flex flex-col">
      ...
      <div className="flex flex-col gap-4 flex-1 overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-white/20 scrollbar-track-transparent">
        ...
        {tools.map(tool => (
          <Button
            ...
            onClick={() => {
              console.log(`[DrawingPanel Click] Button clicked for tool: ${tool.id}. Editor available:`, !!editor);
              if (editor?.setCurrentTool) {
                try {
                  console.log(`[DrawingPanel Click] Attempting editor.setCurrentTool('${tool.id}')`);
                  editor.setCurrentTool(tool.id);
                  setCurrentTool(tool.id);
                } catch (e) {
                  console.error(`[DrawingPanel Click] Failed to set tool to ${tool.id}:`, e);
                }
              } else {
                console.error("[DrawingPanel Click] Editor or setCurrentTool method not available");
              }
            }}
          >
            {tool.icon}
      ...
      {currentTool === 'geo' && (
          ...
          <div className="flex flex-wrap gap-1">
            {geoShapes.map(shape => (
              <Button
                ...
                variant="ghost"
                className={`h-auto p-1.5 flex items-center gap-1.5 text-xs transition-colors 
                  ${editor?.getInstanceState().stylesForNextShape.geo === shape.id
            ...
            {colors.map(color => (
              <button
                ...
                className={`w-6 h-6 rounded-full transition-transform ${
                  currentColor === color.value ? 'scale-110 ring-2 ring-white ring-offset-2 ring-offset-[rgba(24,24,26,0.6)]' : 'hover:scale-105'
                }`}
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex items-center gap-1">
            {opacityLevels.map(opacity => (
              ...
              >
                <div className="w-4 h-4 rounded-full border border-current" style={{ opacity }} />
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="mt-auto pt-3 border-t border-white/10 space-y-1 text-[10px] text-white/50">
          <div className="font-medium text-xs text-white/70 mb-1.5">Keyboard Shortcuts</div>
          <div className="grid grid-cols-2 gap-x-4 gap-y-1">
            <div className="flex justify-between">
              <span>Select Tool</span>
              <kbd className="px-1 py-0.5 bg-black/20 rounded text-[9px]">V</kbd>
            </div>
            <div className="flex justify-between">
              <span>Pen Tool</span>
              <kbd className="px-1 py-0.5 bg-black/20 rounded text-[9px]">D</kbd>
            </div>
            <div className="flex justify-between">
              <span>Eraser</span>
              <kbd className="px-1 py-0.5 bg-black/20 rounded text-[9px]">E</kbd>
            </div>
            <div className="flex justify-between">
              <span>Shapes</span>
              <kbd className="px-1 py-0.5 bg-black/20 rounded text-[9px]">G</kbd>
            </div>
            <div className="flex justify-between">
              <span>Rectangle</span>
              <kbd className="px-1 py-0.5 bg-black/20 rounded text-[9px]">R</kbd>
            </div>
            <div className="flex justify-between">
              <span>Ellipse</span>
              <kbd className="px-1 py-0.5 bg-black/20 rounded text-[9px]">O</kbd>
            </div>
            <div className="flex justify-between">
              <span>Undo</span>
              <kbd className="px-1 py-0.5 bg-black/20 rounded text-[9px]">⌘Z</kbd>
            </div>
            <div className="flex justify-between">
              <span>Redo</span>
              <kbd className="px-1 py-0.5 bg-black/20 rounded text-[9px]">⌘⇧Z</kbd>
...
const RecordingsPanel = ({ recordings, onSelect }: { recordings: Recording[], onSelect: (rec: Recording) => void }) => {
  return (
    <div className="p-3 text-white/70 text-sm h-full flex flex-col">
              ...
              {recordings.slice().reverse().map(rec => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                       <Video size={14} className="text-white/40 group-hover:text-white/70 transition-colors flex-shrink-0" />
                       <span className="truncate" title={rec.name}>{rec.name}</span>
                    </div>
...
const InlineChatPanel = () => {
  ...
  const sendMessage = () => {
    ...
    setTimeout(() => {
      setMessages(prev => [...prev, {
        role: 'assistant',
        content: 'This is a mock response. The actual AI integration will be implemented later.',
        timestamp: Date.now()
      }]);
    }, 1000);
...
export const Toolbar = () => {
  ...
  const [activeTools, setActiveTools] = useState<Set<ToggleableTools>>(new Set());
  ...
  const [visibleToolbarItems, setVisibleToolbarItems] = useState<string[]>(() => {
    try {
      ...
      return saved ? JSON.parse(saved) : DEFAULT_TOOLBAR_ITEMS.map(item => item.id);
    } catch (error) {
  ...
  const [consoleLogs, setConsoleLogs] = useState<Array<{
    ...
    { id: 3, type: 'warn', content: 'Resource interpreted as Script but transferred with MIME type text/plain', timestamp: '10:42:18' },
  ...
  useEffect(() => {
    ...
    const intercept = (type: 'log' | 'info' | 'warn' | 'error', args: any[]) => {
  ...
  const cleanupActiveRecording = useCallback(() => {
    ...
    if (mediaRecorderRef.current) {
      ...
      if (mediaRecorderRef.current.state !== "inactive") {
        try { mediaRecorderRef.current.stop(); } catch (e) { console.warn("Cleanup stop error:", e); }
      }
  ...
  const stopRecording = useCallback((wasStoppedExternally = false) => {
    if (!mediaRecorderRef.current || mediaRecorderRef.current.state === "inactive") {
       ...
       if(recordingState !== 'idle' && recordingState !== 'recorded') {
         ...
         setActiveTools(prev => { const n = new Set(prev); n.delete("record"); return n; });
       }
       return;
    }
  ...
  const startRecording = useCallback(async () => {
    if (mediaStreamRef.current && mediaStreamRef.current.active) {
        console.warn("Start recording blocked: Media stream already exists and is active.");
        return;
    }
    
    if (recordingState !== "idle" && recordingState !== "error" && recordingState !== "recorded") {
        console.warn(`Start recording blocked: Current state is ${recordingState}`);
        return;
    }
    ...
    try {
      ...
      try { 
          ...
          try { 
              console.log("Attempting to create MediaRecorder with default options.");
              recorder = new MediaRecorder(stream); 
          } 
          catch (fallbackError: any) { 
              console.error("Fallback MediaRecorder creation failed:", fallbackError);
              throw new Error(`MediaRecorder init failed: ${fallbackError?.message}`); 
          }
      }
      ...
      recorder.onstop = () => {
        console.log("MediaRecorder onstop event triggered. Final chunks collected:", recordedChunksRef.current.length);
        ...
        const blob = new Blob(currentChunks, { type: recorder.mimeType || "video/webm" });
        ...
        console.log("Setting recording to play immediately:", newRecording.name);
      ...
      if (err instanceof Error) {
        if (err.name === "NotAllowedError") message = "Permission denied. Please allow screen recording access.";
  ...
  useEffect(() => {
    return () => {
      ...
      if (playingRecording && !recordings.some(r => r.url === playingRecording.url)) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem(TOOLBAR_CONFIG_KEY, JSON.stringify(visibleToolbarItems));
    } catch (error) {
      ...
      if (wasActive) {
        ...
        if (tool === "record") {
          if (recordingState === "idle" || recordingState === "error" || recordingState === "recorded") {
              ...
              console.warn(`Record toggle ON ignored: State is already ${recordingState}`);
          }
        }
        setExpanded(true); 
      }
  ...
  const showRecordPanel = activeSection === 'record' || activeTools.has('record');
  const showCombinedPanel = showDrawPanel && showRecordPanel;
  const showAnyToolPanel = showDrawPanel || showRecordPanel;
  const isPanelVisible = !playingRecording && expanded && (activeSection || showAnyToolPanel);
  ...
  const backdropStyle = "backdrop-blur-xl bg-[rgba(24,24,26,0.6)] border border-[rgba(255,255,255,0.05)]";
  const shadowStyle = "shadow-[0_8px_32px_rgba(0,0,0,0.12)] hover:shadow-[0_8px_36px_rgba(0,0,0,0.18)]";
  ...
  const isRecordingOrStarting = recordingState === "recording" || recordingState === "starting";
  ...
  useEffect(() => {
    ...
    window.addEventListener('chat-visibility-change', handleChatVisibilityChange as EventListener);
    return () => {
      window.removeEventListener('chat-visibility-change', handleChatVisibilityChange as EventListener);
    };
  }, []);
  ...
  const generateMockConsoleLogs = (count: number) => {
    ...
    const types: Array<'log' | 'info' | 'warn' | 'error'> = ['log', 'info', 'warn', 'error'];
    const components = ['App', 'Router', 'Component', 'Button', 'Modal', 'Form', 'Auth', 'API', 'Store', 'Renderer'];
    const actions = ['initialized', 'updated', 'rendered', 'mounted', 'unmounted', 'failed', 'loaded', 'changed', 'processed', 'connected'];
    const errors = [
    ...
    for (let i = 0; i < count; i++) {
      ...
      const timestamp = new Date(now - Math.random() * 3600000).toLocaleTimeString();
      ...
      if (type === 'error') {
        ...
        if (Math.random() > 0.7) {
          content += '\n    at ' + components[Math.floor(Math.random() * components.length)] + 
                    '.js:' + Math.floor(Math.random() * 1000) + ':' + Math.floor(Math.random() * 100);
        }
      } else if (type === 'warn') {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      console.log("[Toolbar KeyDown] Editor instance available:", !!currentEditor);
      if (!currentEditor) return;
      
      const shortcuts: Record<string, () => void> = {
        v: () => { if (currentEditor.setCurrentTool) { console.log("[Toolbar KeyDown] Setting tool: select"); currentEditor.setCurrentTool('select'); } },
        d: () => { if (currentEditor.setCurrentTool) { console.log("[Toolbar KeyDown] Setting tool: draw"); currentEditor.setCurrentTool('draw'); } },
        e: () => { if (currentEditor.setCurrentTool) { console.log("[Toolbar KeyDown] Setting tool: eraser"); currentEditor.setCurrentTool('eraser'); } },
        g: () => { if (currentEditor.setCurrentTool) { console.log("[Toolbar KeyDown] Setting tool: geo"); currentEditor.setCurrentTool('geo'); } },
        r: () => {
  ...
  return (
    <>
      {drawingCanvasVisible && (
        <div 
          style={{ position: "fixed", inset: 0, pointerEvents: "auto", zIndex: 60, background: "transparent" }} 
        ...
        >
          ...
          <Tldraw 
            ...
            onMount={(editor) => { 
              console.log("[Toolbar Tldraw] onMount - Editor instance created:", editor);
      ...
      >
        ...
        <motion.div 
           ...
           transition={{ type: "spring", stiffness: 400, damping: 35, delay: expanded ? 0.1 : 0 }} 
        ...
        >
          <div className="flex h-10 items-center bg-[rgba(30,30,34,0.55)] flex-shrink-0">
            <div className="flex items-center border-l border-r border-[rgba(255,255,255,0.04)] h-full">
              ...
              >
                <div className="flex items-center justify-center w-6 h-6">
                  <Circle size={14} className={isRecordingActive ? "fill-red-500 text-red-500" : (activeTools.has("record") ? "text-red-400" : "")} />
                </div>
              </Button>
              <Button
            ...
            <div className="flex h-full">
              {DEFAULT_TOOLBAR_ITEMS.filter(item => visibleToolbarItems.includes(item.id)).map(item => (
                <Button
                  key={item.id}
                  ...
                  className={`flex items-center rounded-none text-xs px-2 h-full transition-colors relative whitespace-nowrap ${
                    activeSection === item.id ? "text-white bg-[rgba(50,50,56,0.7)]" : "text-[#A1A1A6] hover:text-white"
                  }`}
                >
                  <span className="mr-1 flex-shrink-0">{item.icon}</span>
                  <span>{item.id === "record" ? `${item.label} (${recordings.length})` : item.label}</span>
          ...
          >
              <AnimatePresence mode="wait" initial={false}>
                 {isPanelVisible && (
                   <motion.div 
                      key={showCombinedPanel ? 'combined' : activeSection || (showDrawPanel ? 'draw' : 'record') || 'none'} 
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                      exit={{ opacity: 0 }}
                    ...
                    >
                      {showCombinedPanel ? (
                          <ResizablePanelGroup direction="horizontal" className="h-full">
                              ...
                              <ResizablePanel defaultSize={50} minSize={30}> <RecordingsPanel recordings={recordings} onSelect={handleSelectRecording} /> </ResizablePanel>
                          </ResizablePanelGroup>
                      ) : showDrawPanel ? (
                          <DrawingPanel editor={editorInstance} />
                      ) : showRecordPanel ? (
                         <RecordingsPanel recordings={recordings} onSelect={handleSelectRecording} />
                      ) : activeSection === "select" ? (
                          <ResizablePanelGroup direction="horizontal" className="h-full">
                            <ResizablePanel defaultSize={chatVisible ? 60 : 100} minSize={40} className="h-full">
                              <div className="p-3 text-white/70 text-sm h-full flex flex-col">
                                <div className="font-medium mb-2 text-white flex justify-between items-center">
                                  ...
                                  <div className="flex items-center gap-2">
                                    <Button
                                      variant="ghost"
                                      size="sm"
                                      onClick={() => {/* TODO: Implement add to chat */}}
                                    ...
                                    <Button
                                      variant="ghost"
                                      size="sm"
                                      onClick={() => setChatVisible((prev: boolean) => !prev)}
                              ...
                              <div className="font-medium mb-2 text-white flex justify-between items-center">
                                ...
                                <div className="flex items-center gap-2">
                                  <Button
                                    ...
                                    size="sm"
                                    onClick={() => {
                                      const mockLogs = generateMockConsoleLogs(1000);
                          ...
                          <ResizablePanel defaultSize={chatVisible ? 60 : 100} minSize={40} className="h-full">
                            <div className="p-3 text-white/70 text-sm h-full flex flex-col">
                              ...
                              <div className="flex-1 text-xs opacity-70">
                                <div className="bg-black/10 p-2 rounded mt-2 h-full overflow-y-auto">...</div>
                              </div>
                            </div>
                          </ResizablePanel>
                          {chatVisible && (
                        ...
                        <ResizablePanelGroup direction="horizontal" className="h-full">
                            ...
                            <div className="p-3 text-white/70 text-sm h-full flex flex-col">
                              ...
                              <div className="grid grid-cols-2 gap-2 mb-3">
                                <div className="bg-black/20 rounded p-2">
                                  <div className="text-[10px] text-white/50 mb-1">CPU Usage</div>
                                  <div className="flex items-end gap-1">
                                    <div className="text-lg font-medium text-white">23%</div>
                                    <div className="text-[10px] text-green-400 mb-1">↓ 2%</div>
                                  </div>
                                </div>
                                <div className="bg-black/20 rounded p-2">
                                  <div className="text-[10px] text-white/50 mb-1">Memory</div>
                                  <div className="flex items-end gap-1">
                                    <div className="text-lg font-medium text-white">1.2GB</div>
                                    <div className="text-[10px] text-red-400 mb-1">↑ 50MB</div>
                                  </div>
                                </div>
                              </div>
                              <div className="flex-1 overflow-hidden flex flex-col gap-3">
                                <div>
                                  <div className="text-xs font-medium mb-1.5 text-white/90">Resource Timeline</div>
                                ...
                                <div>
                                  <div className="text-xs font-medium mb-1.5 text-white/90">Active Processes</div>
                                  <div className="space-y-1">
                                    <div className="bg-black/20 rounded p-2 flex items-center justify-between">
                                      <div className="text-xs">Main Process</div>
                                      <div className="text-[10px] text-white/50">120MB</div>
                                    </div>
                                    <div className="bg-black/20 rounded p-2 flex items-center justify-between">
                                      <div className="text-xs">Renderer</div>
                                      <div className="text-[10px] text-white/50">450MB</div>
                                    </div>
                                    <div className="bg-black/20 rounded p-2 flex items-center justify-between">
                                      <div className="text-xs">GPU Process</div>
                                      <div className="text-[10px] text-white/50">280MB</div>
                                    </div>
                                  </div>
                                </div>
                              </div>
                          ...
                          {chatVisible && (
                           ...
                           <div className="p-3 text-white/70 text-sm h-full"><div className="font-medium mb-2 text-white">Additional Tools</div><div className="text-xs opacity-70">...</div></div> 
                        ...
                        <div className="p-3 text-white/70 text-sm h-full flex flex-col">
                          ...
                          <div className="flex-1 overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-white/20 scrollbar-track-transparent">
                            <div className="space-y-4">
                              <div>
                                <h4 className="font-medium mb-2 text-xs text-white/90">Toolbar Items</h4>
                                <div className="space-y-1">
                                  {DEFAULT_TOOLBAR_ITEMS.map(item => (
                                    ...
                                    >
                                      <input
                                        type="checkbox"
                                        checked={visibleToolbarItems.includes(item.id)}
                                        onChange={() => handleToggleToolbarItem(item.id)}
                                        className="rounded border-white/20 bg-white/10 checked:bg-blue-500 checked:border-0 transition-colors"
                                      />
                                      <span className="flex items-center gap-1.5">
                                        <span className={`transition-colors ${visibleToolbarItems.includes(item.id) ? 'text-white' : 'text-white/50 group-hover:text-white/70'}`}>
                                          {item.icon}
                                        </span>
                                        <span className={`text-xs transition-colors ${visibleToolbarItems.includes(item.id) ? 'text-white' : 'text-white/50 group-hover:text-white/70'}`}>
                                          {item.label}
                                        </span>
                                      </span>
                                    </label>
                              ...
                              <div className="pt-2 border-t border-white/10">
                                <Button
                                  ...
                                  onClick={() => setVisibleToolbarItems(DEFAULT_TOOLBAR_ITEMS.map(item => item.id))}
                                >
                                  Reset to Default
                                </Button>
                              </div>
      ...
      {recordingState === "error" && error && (
        ...
        >
          ...
          <Button variant="ghost" size="icon" className="h-5 w-5 text-white/70 hover:text-white hover:bg-white/10" onClick={() => {setError(null); setRecordingState("idle");}}><X size={12}/></Button>
        </motion.div>
      )}
